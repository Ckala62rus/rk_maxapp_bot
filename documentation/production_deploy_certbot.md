...
Если на сервере порты `80`/`443` уже заняты, в `.env` можно задать `WEB_HTTP_PORT` и `WEB_HTTPS_PORT` (например `88` и `8443`) — см. `README.md`. Учти: для **HTTP-01** Let’s Encrypt на домен обычно должен отвечать **внешний порт 80**; если он недоступен, нужен **DNS-01**, общий reverse proxy на 80 или временное освобождение порта.
